...
// Closed Tickets is Backlog filtered to Done, as its own page (not just
// a Backlog filter) — DesignBrief.md: closed tickets are "done with,"
// not part of active-work triage. Status filter/column hidden since
// it's always Done here.
export function ClosedTicketsPage() {
  ...
  const { data: issues } = useQuery({
    queryKey: ["issues", projectId, "closed", filters, sort],
    queryFn: () =>
          ...
          {issues?.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-2 py-16">
              <span className="font-body text-[15px] text-ink-soft">
                {hasActiveFilters ? "No tickets match your filters." : "No closed tickets yet."}
              </span>
              {hasActiveFilters && (
                <button
